...
def main(path):
    ...
    while True :
        ...
        if tocke[trenutni.x - 1][trenutni.y] != -1 and [trenutni.x - 1, trenutni.y] not in obiskani:
            novnode = Node(trenutni.x - 1, trenutni.y, trenutni, tocke[trenutni.x - 1][trenutni.y], konci)
            sestevek = novnode.getWeight() + novnode.getRazdalja()
            naslednji.append([sestevek, novnode])
        ...
        if tocke[trenutni.x][trenutni.y - 1] != -1 and [trenutni.x, trenutni.y - 1] not in obiskani:
            novnode = Node(trenutni.x, trenutni.y - 1, trenutni, tocke[trenutni.x][trenutni.y - 1], konci)
            sestevek = novnode.getWeight() + novnode.getRazdalja()
            naslednji.append([sestevek, novnode])
            nodes.append(novnode)
    ...
    print("Pozresno hevristicno preiskovanje cena poti: " + str(koncniNode.getWeight()))
    print("Pozresno hevristicno preiskovanje dolzina poti: " + str(len(resitev) - 1))
    print("Pozresno hevristicno preiskovanje pregledanih vozlisc: " + str(len(nodes)))
